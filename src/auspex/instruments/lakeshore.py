...
# Message checkers
	...
	def check_htr_msg(self, vals):
		...
		if vals[1] not in self.ZO_VALS.append(2):
...
# Heater and PID Control
	...
	def pid_htr_1(self, vals):
		...
		self.interface.write(("PID 1,"+','.join(['{:E}']*len(vals))).format(*vals))
	...
	def pid_htr_2(self, vals):
		self.check_pid_msg(vals)
		self.interface.write(("PID 2,"+','.join(['{:E}']*len(vals))).format(*vals))
	...
	def temp_htr_1(self, val):
		self.interface.write("SETP 1,'{:E}'".format(val)) 
	...
	def temp_htr_2(self, val):
		self.interface.write("SETP 2,'{:E}'".format(val)) 
